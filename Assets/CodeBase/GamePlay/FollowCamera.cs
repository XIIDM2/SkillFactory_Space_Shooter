...
public class FollowCamera : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        Vector2 targetPos = m_Target.transform.position + m_Target.transform.up * m_ForwardOffset;

        Vector2 newCamPos = Vector2.Lerp(camPos, targetPos, m_InterpolationLinear * Time.deltaTime);

        transform.position = new Vector3 (newCamPos.x, newCamPos.y, m_CameraOffsetZ);

        if(m_InterpolationAngular > 0)
        {
            transform.rotation = Quaternion.Slerp(transform.rotation, m_Target.transform.rotation, m_InterpolationAngular * Time.deltaTime);
        }
